...
```r
treatment.first(n, base = 1, intercept = FALSE, contrasts = TRUE, sparse = FALSE)
```

### Arguments

- **`n`**: An integer specifying the number of levels, or a character vector of level names.
- **`base`**: An integer indicating which level to use as the baseline (default is 1).
